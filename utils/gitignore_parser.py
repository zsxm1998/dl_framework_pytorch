# copy from https://github.com/mherrmann/gitignore_parser
import collections
import os
import re
...
def ignorer_from_gitignore(full_path, base_dir=None):
    matcher = parse_gitignore(full_path, base_dir)
    def _ignorer(src, names):
        ignored_names = set()
        for name in names:
            if matcher(os.path.join(src, name)) or name == '.git':
